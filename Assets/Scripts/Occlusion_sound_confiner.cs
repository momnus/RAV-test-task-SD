...
public class Occlusion_sound_confiner : MonoBehaviour
{
    [SerializeField] private bool SimpleOcclution;
    [SerializeField] private GameObject PlayerObject; // ссылка на объект плейера для того, чтобы найти дистанции
    [SerializeField] private LayerMask OcclusionLayer; // слой, который блокирует лучи
    [SerializeField] private float SoundWidth; // расстояние до левого и правого векторов у источника звука. Эмитация на сколько большой размер источника звука
    [SerializeField] private float ListenerWidth; // расстояние левого и правого векторов у листенера, компенсация вращения камеры у персонажа
    [SerializeField] private Vector3 SoundHeight; // это высота от куда будут кидаться лучи. Компенсация того, что геймобъект располагается в ногах персонажа
    [SerializeField] private Vector3 PlayerHeight; // То же самое для листенера
